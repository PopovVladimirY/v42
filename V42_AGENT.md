...
#### Вариант C: agent token без срока жизни (Phase D, ещё не реализован)

Когда Phase D будет готова -- в UI появится раздел "Agent Tokens".
Там можно создать именованный токен для конкретного агента без срока жизни.
Его можно отозвать в любой момент. Это правильный способ для проде.
...
Рабочие варианты:
- Переменная окружения в системе Windows: `setx V42_API_TOKEN "eyJ..."` (перманентно)
- В файле `~/v42/.env.local` (добавлен в `.gitignore`):
  ```
  V42_API_TOKEN=eyJ...
  ```
- Напрямую в конфиге клиента -- приемлемо для локальной разработки,
  но только если конфиг не попадает в git (файлы вроде `~/.cursor/mcp.json` -- норм).
