...
Производительность команды (velocity) -- это число, которое ничего не значит без
контекста. Закрыли 10 историй за спринт? Здорово. А если команда работала вполсилы
из-за отпусков и болезней -- это в 2 раза хуже, чем могло бы быть. Или в 2 раза
лучше, чем обычно. Без знания фактической загрузки невозможно понять ни того, ни другого.

Два момента истины в жизни спринта:
1. **Sprint Planning** -- кто будет работать и сколько часов мы планируем с каждого.
2. **Sprint Review** -- кто фактически работал и сколько. Отпуска, болезни, форс-мажоры.

Разрыв между планом и фактом -- это и есть сигнал для нормализации.
...
| Что | Где | Ограничение |
|-----|-----|-------------|
| `team_members.capacity_hours` | DB | Нормативная недельная загрузка участника. Не привязана к конкретному спринту. |
| `sprints.capacity_hours` | DB | Суммарный план на спринт. Одно число, нет детализации по участникам. |
| `GET /projects/{id}/capacity` | API | Skill-radar: сколько часов каждого скила есть в команде. Это другое -- это структурный капасити команды, не спринтовый. |

---

### Новая модель: `sprint_capacity`

Нужна отдельная таблица -- план и факт по каждому участнику в каждом спринте.
...
CREATE TABLE sprint_capacity (
    ...
    planned_hours  NUMERIC(5,1) NOT NULL DEFAULT 0,  -- set at sprint planning
    actual_hours   NUMERIC(5,1) NULL,                 -- set at sprint review; NULL until finalized
    notes          TEXT,                              -- "on vacation days 3-5"
...
`sprints.capacity_hours` остаётся как денормализованный кэш суммы `planned_hours`
(обновляется при изменении строк `sprint_capacity`). Это позволяет не трогать
существующие API-контракты.

---

### Агрегация: скилы vs подкоманды

Вопрос: по чему агрегировать капасити спринта -- по скилам или по подкомандам?

**Ответ: скилы как первичное измерение, команды как вторичный фильтр.**

Обоснование:
- `member_skills` уже есть. Запрос "сколько Go-часов у нас в этом спринте?" --
  это JOIN `sprint_capacity → member_skills GROUP BY skill_id`. Новых таблиц ноль.
- Командный срез: `sprints.team_id` плюс `project_teams` уже дают разбивку
  по команде без дополнительных структур. Нужен -- берём.
- Скиловый срез actionable: "У нас 60h Go и 20h QA -- можем ли мы взять этот эпик?"
  Команды такого ответа не дают.

Пример агрегирующего запроса (sqlc):
```sql
-- Capacity by skill for a sprint
SELECT s.id, s.name, SUM(sc.planned_hours) AS planned_hours
...
Все маршруты под `/projects/{id}/sprints/{sid}/capacity`.

| Метод | Путь | Когда | Описание |
|-------|------|-------|----------|
| `GET` | `/projects/{id}/sprints/{sid}/capacity` | Любой | Список участников: planned, actual (null пока не закрыт спринт), skill breakdown |
| `PUT` | `/projects/{id}/sprints/{sid}/capacity` | Planning | Bulk upsert: массив `{user_id, planned_hours, notes}` |
| `PATCH` | `/projects/{id}/sprints/{sid}/capacity/{uid}` | Review | Фиксация факта: `{actual_hours, notes}` |
| `POST` | `/projects/{id}/sprints/{sid}/capacity/init` | Planning | Автозаполнение из `team_members.capacity_hours` (умноженный на длину спринта в неделях) |
| `GET` | `/projects/{id}/velocity` | Analytics | История нормализованной скорости по спринтам |

Ответ `GET capacity`:
```json
{
  "sprint_id": "...",
  "total_planned_hours": 160,
  "total_actual_hours": 128,
  "utilization_pct": 80,
  "members": [
    {
      "user_id": "...", "display_name": "Ivan",
      "planned_hours": 40, "actual_hours": 32, "notes": "sick day 2",
      "skills": [
  ...
  "by_skill": [
    { "skill_id": "...", "skill": "Go", "planned_hours": 80, "actual_hours": 64 }
  ]
}
```
...
**Сырая velocity:** количество закрытых items / story points за спринт.

**Нормализованная velocity:** `completed_items / actual_capacity_hours * 100`
(items на 100 person-hours). Сравнима между спринтами независимо от:
- Размера команды (взяли нового человека → capacity выросла, raw velocity выросла,
  нормализованная остаётся сопоставимой)
- Длины спринта (2 недели vs 3 недели)
- Фактической доступности (отпуска, больничные)

Хранение: при финализации спринта (`PATCH /sprints/{sid}` с `status=completed`)
записываем `actual_capacity_hours` как `SUM(actual_hours)` и `velocity_normalized`
в таблицу `sprints` (два новых денормализованных поля, либо вычисляем on-the-fly).

```sql
-- Sprint velocity history for project
SELECT
    s.sprint_number,
    s.name,
    COUNT(si.backlog_item_id) FILTER (
        WHERE bi.status = 'done'
    ) AS completed_items,
    SUM(sc.actual_hours) AS actual_hours,
    ROUND(
        COUNT(si.backlog_item_id) FILTER (WHERE bi.status = 'done')
        * 100.0 / NULLIF(SUM(sc.actual_hours), 0),
        2
    ) AS velocity_normalized
FROM sprints s
...
GROUP BY s.id, s.sprint_number, s.name
...
```
Sprint Planning
  └── PUT /capacity (bulk)         ← команда заполняет кто будет работать
  └── POST /capacity/init          ← или автозаполнение из team_members

Sprint Active
  └── GET /capacity                ← read-only: план зафиксирован, актив не заполнен

Sprint Review
  └── PATCH /capacity/{uid}        ← корректируем по каждому участнику
  └── POST /sprints/{sid} (status=completed)
                                    ← финализация: блокирует capacity, пересчитывает velocity
```
...
Маршрут: `/projects/:id/analytics`

Компоненты (Recharts, уже в стеке):

**VelocityChart** -- ComposedChart:
- Bar: `actual_capacity_hours` (серый, фон)
- Line: `velocity_normalized` (синяя линия)
- X-axis: sprint number / name
- Tooltip: спринт, закрытые items, actual hours, normalized score

**CapacityChart** -- GroupedBarChart:
- Два бара на спринт: planned (светлый) vs actual (тёмный)
- Линия: utilization_pct (%)
- Сразу видно: какие спринты были "короткими" по факту

**SkillBreakdownChart** -- StackedBarChart:
- X: спринты, Y: часы, цвет: скил
- Отвечает на вопрос "по каким скилам мы исторически перегружены/недогружены"

**TeamCapacityTable** -- агрегированная таблица:
- Участник | AVG planned/sprint | AVG actual/sprint | Delta | Trend (мини-sparkline)
...
**Фаза: Sprint Analytics** (после текущего Phase 4, до Phase 5 Releases)

#### Backend

| Шаг | Что | Сложность |
|-----|-----|-----------|
| 1 | Migration 000020: `sprint_capacity` table | Просто |
| 2 | sqlc queries: `UpsertSprintCapacity`, `ListSprintCapacity`, `BulkInitCapacity`, `GetVelocityHistory`, `GetSkillCapacityBySprint` | Умеренно |
| 3 | Handler: новый файл `handler_sprint_capacity.go` или расширить `handler_tasks_sprints.go` | Умеренно |
| 4 | Routes: `GET/PUT/PATCH /projects/{id}/sprints/{sid}/capacity`, `POST /init`, `GET /projects/{id}/velocity` | Просто |
| 5 | Sprint finalization logic: при `status=completed` вычислить и кэшировать `actual_capacity_hours` | Умеренно |

#### Frontend

| Шаг | Что | Сложность |
|-----|-----|-----------|
| 1 | Hook `useSprintCapacity(sprintId)` -- GET + PUT + PATCH | Просто |
| 2 | Hook `useProjectVelocity(projectId)` -- GET velocity history | Просто |
| 3 | `SprintCapacityPanel` компонент (3 режима: planning / active / review) | Умеренно |
| 4 | `ProjectAnalyticsPage` с VelocityChart + CapacityChart | Умеренно |
| 5 | `SkillBreakdownChart` + `TeamCapacityTable` | Просто |
...
## Navigation: Sidebar, My Sprints & Session Persistence

### Постановка задачи

Три связанных UX-улучшения:
1. **My Active Sprints** -- прямой линк в сайдбаре на все активные спринты пользователя
   (его команды). Не надо бродить через проекты чтобы найти где работаешь.
2. **Recent Sprints в сайдбаре** -- быстрый доступ к последним посещённым спринтам.
3. **Session persistence** -- пользователь всегда возвращается в то место и состояние,
   на котором закончил предыдущую сессию.

---

### Sidebar: финальная структура

```
┌─────────────────────────────┐
│  [V42]      [user avatar]   │
├─────────────────────────────┤
│  My Sprints          [badge]│  ← НОВОЕ: /sprints (кол-во active)
│  Dashboard                  │
├─────────────────────────────┤
│  RECENT SPRINTS             │  ← НОВОЕ: localStorage, max 5
│  › Sprint 14 · Alpha        │
│  › Sprint 3 · Phoenix       │
│  › Sprint 7 · Core          │
├─────────────────────────────┤
...
Сайдбар collapsible. Состояние (свёрнут/развёрнут) персистируется в localStorage.

---

### My Active Sprints (`/sprints`)

Отдельная страница -- точка входа в рабочий процесс. Не проекты, не команды -- а
именно "где я работаю прямо сейчас."

**Что видит пользователь:**
- Все спринты со `status = active` в проектах, где он состоит хотя бы в одной команде.
- Admin видит все активные спринты без фильтра.

**Карточка спринта:**
```
┌──────────────────────────────────────────┐
│  Sprint 14                    [ACTIVE]   │
│  Project Alpha · Team Backend            │
│  May 19 – Jun 1 · 9 days left           │
│                                          │
│  Items:  ████████░░  14/20 done          │
│  Capacity: 80h planned / 64h actual      │
│                                          │
│  [Board]  [Backlog]  [Tests]  [Capacity] │
└──────────────────────────────────────────┘
```

Клик на кнопки → сразу в нужную вкладку спринта. Клик на заголовок → страница спринта
на последней открытой вкладке (из recent-state).

**Группировка:** по умолчанию по проекту. Переключатель: Flat list / By project / By team.
Предпочтение сохраняется в `ui_settings`.

**Дополнительный раздел: Planning** (status = planning) -- свёрнутый аккордеон.
Спринты в планировании видны, но не в основной секции. "Вот что скоро начнётся."

#### API

Новый top-level endpoint:

```
GET /sprints?status=active&my=true
GET /sprints?status=active              # admin: все active спринты системы
GET /sprints?status=planning&my=true   # планируемые спринты пользователя
```

Backend-запрос (основа для sqlc):
```sql
...
WHERE s.status = $1
  AND (
    ...
    OR EXISTS (
        SELECT 1 FROM project_teams pt
        JOIN team_members tm ON tm.team_id = pt.team_id
        WHERE pt.project_id = s.project_id AND tm.user_id = $user_id
    )
  )
GROUP BY s.id, p.name, t.name
ORDER BY p.name, s.sprint_number;
```

---

### Recent Sprints (в сайдбаре)

**Хранение:** `localStorage` ключ `v42:recent_sprints`, массив max 5 объектов.

```ts
interface RecentSprint {
  sprint_id: string;
  sprint_name: string;
  project_id: string;
  project_name: string;
...
**Обновление:** при каждом переходе на страницу спринта. Дубликаты вытесняются
(sprint_id уже есть → обновить `visited_at` и `last_tab`, поднять в топ).

**Отображение:** только имя спринта + проект. Тап → сразу на `last_tab` этого спринта.

Zustand-store `useNavigationStore` с `persist` middleware (localStorage):
```ts
interface NavigationState {
  recentSprints: RecentSprint[];
  addRecentSprint: (sprint: RecentSprint) => void;
  lastRoute: string;
...
**Принцип:** URL -- источник истины для состояния вьюхи. Zustand/localStorage --
для мета-состояния (что посещал, куда вернуться, персональные настройки).

#### URL как состояние (view state in URL)

Все значимые состояния вьюхи живут в URL -- тогда ссылку можно передать коллеге,
и он откроет ровно то же самое.

| Состояние | Где в URL | Пример |
|-----------|-----------|--------|
| Активная вкладка спринта | path segment | `/projects/:id/sprints/:sid/board` |
| Фильтры бэклога | search params | `?status=ready&epic=abc` |
| Группировка бэклога | search param | `?group=epic` |
| Активная вкладка проекта | path segment | `/projects/:id/backlog` |

React Router v6 + `useSearchParams` -- всё уже в стеке.

#### Последний маршрут (между сессиями)

При каждом изменении маршрута:
```ts
// в router listener (React Router v6)
router.subscribe(({ location }) => {
  navigationStore.setLastRoute(location.pathname + location.search);
});
```

При логине (успешный `POST /auth/login`):
```ts
const lastRoute = navigationStore.lastRoute;
if (lastRoute && lastRoute !== '/login') {
  navigate(lastRoute, { replace: true });
} else {
  navigate('/sprints');  // default landing
}
```

При refresh (AutoLogin / токен ещё жив):
- Пользователь открыл вкладку → приложение валидирует токен (`GET /auth/me`) →
  роутер остаётся на текущем URL (не редиректит) → состояние восстановлено.

#### Per-entity preferences (server-side)

Предпочтения, которые должны переживать очистку браузера или вход с другого
устройства, хранятся в `users.ui_settings` (JSONB, новое поле, migration 000021).

```json
{
  "default_sprint_tab": "board",
  "sprint_group_by": "by_project",
  "backlog_group_by": "epic",
  "backlog_default_filter": "ready",
...
Body: { "default_sprint_tab": "backlog" }
```

Загружаются вместе с `GET /auth/me` (включены в ответ). Таким образом при первом
рендере уже известны предпочтения -- нет прыжка между состояниями.

**Стратегия слияния:** localStorage имеет приоритет над `ui_settings` для
сессионного состояния (текущие фильтры). `ui_settings` -- дефолты между сессиями.

```
При старте приложения:
  1. Загрузить ui_settings из GET /auth/me
  2. Применить как дефолты в Zustand store
  3. Если localStorage имеет более свежее состояние (по timestamp) -- взять его
  4. Navigate to lastRoute (из localStorage)
```

#### Scroll position

Для длинных страниц (бэклог, история спринтов): сохранять `scrollY` в sessionStorage
(только для текущей вкладки браузера, не между сессиями). Восстанавливать при
навигации назад (browser Back).

React Router v6 `scrollRestoration` или кастомный `useScrollRestore(key)` hook.

---

### Схема данных: изменения

| Что | Migration | Изменение |
|-----|-----------|-----------|
| `users.ui_settings` | 000021 | `JSONB NULL DEFAULT NULL` -- персональные UI-предпочтения |
| `users.last_active_at` | 000021 | `TIMESTAMPTZ NULL` -- для аналитики и "online" индикатора |
...
`last_active_at` обновляется middleware на каждый аутентифицированный запрос
(но не чаще раза в минуту -- throttle в middleware, не в БД).
...
| Метод | Путь | Что |
|-------|------|-----|
| `GET` | `/sprints?status=active&my=true` | My Active Sprints (новый top-level endpoint) |
| `GET` | `/sprints?status=planning&my=true` | My Planning Sprints |
| `PATCH` | `/users/me/settings` | Обновить ui_settings (partial update merge) |
| `GET` | `/auth/me` | Расширить ответ: включить `ui_settings` |
...
| Шаг | Что | Сложность |
|-----|-----|-----------|
| 1 | Migration 000021: `ui_settings`, `last_active_at` | Тривиально |
| 2 | sqlc: `ListActiveSprintsForUser`, `ListActiveSprintsAll`, `UpdateUserSettings` | Умеренно |
| 3 | Handler: `GET /sprints` в новом `handler_sprints_global.go` или `handler_tasks_sprints.go` | Умеренно |
| 4 | Handler: `PATCH /users/me/settings` -- partial JSON merge | Просто |
| 5 | Middleware: throttled `last_active_at` update | Просто |
| 6 | `GET /auth/me`: добавить `ui_settings` в ответ | Тривиально |

#### Frontend

| Шаг | Что | Сложность |
|-----|-----|-----------|
| 1 | `useNavigationStore` (Zustand + persist): lastRoute, recentSprints, sidebarCollapsed | Просто |
| 2 | `useUiSettings` hook: загрузка из /auth/me, PATCH /users/me/settings | Просто |
| 3 | Router listener: обновлять lastRoute + recentSprints при навигации | Просто |
| 4 | Login redirect: `navigate(lastRoute \|\| '/sprints')` | Тривиально |
| 5 | Sidebar: секция "Recent Sprints", линк "My Sprints" с badge | Умеренно |
| 6 | `MySprintsPage` (`/sprints`): карточки спринтов, группировка, фильтры | Умеренно |
| 7 | Sprint page: tab state в URL path (board/backlog/tests/capacity) | Просто |
| 8 | Backlog page: фильтры и group-by в search params | Просто |
